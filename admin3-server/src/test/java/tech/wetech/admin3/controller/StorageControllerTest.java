...
public class StorageControllerTest extends AbstractIntegrationTest {
  ...
  void testFindConfigList() throws Exception {
    mvc.perform(get("/storage/configs").header(TOKEN_HEADER_NAME, TOKEN))
  ...
  void testUpload() throws Exception {
    MockMultipartFile file =
        new MockMultipartFile(
            "files", "hello.txt", MediaType.TEXT_PLAIN_VALUE, "Hello, World!".getBytes());
    mvc.perform(multipart("/storage/upload").file(file).header(TOKEN_HEADER_NAME, TOKEN))
        .andExpect(status().isOk());
  }
  ...
  void testUploadToMinIO() throws Exception {
    ...
    mvc.perform(get("/storage/download/{key}", fileKey).header(TOKEN_HEADER_NAME, TOKEN))
        .andExpect(status().isOk());
  ...
  void testCreateAndThenUpdateElseEndDelete() throws Exception {
    ...
    mvc.perform(delete("/storage/configs/{id}", id).header(TOKEN_HEADER_NAME, TOKEN))
  ...
  void testMarkAsDefaultStorageConfig() throws Exception {
    mvc.perform(
            post("/storage/configs/{id}:markAsDefault", 1)
                .contentType(MediaType.APPLICATION_JSON)
                .header(TOKEN_HEADER_NAME, TOKEN))
        .andExpect(status().isNoContent());
  }
