...
public class UserControllerTest extends AbstractIntegrationTest {
  ...
  void testFindUsers() throws Exception {
    mvc.perform(get("/users").header(TOKEN_HEADER_NAME, TOKEN))
  ...
  void testDeleteUser() throws Exception {
    mvc.perform(delete("/users/{userId}", 301).header(TOKEN_HEADER_NAME, TOKEN))
        .andExpect(status().isNoContent());
  }

  @Test
  void testDisableUser() throws Exception {
    mvc.perform(post("/users/{userId}:disable", 302).header(TOKEN_HEADER_NAME, TOKEN))
        .andExpect(status().isOk());
  }

  @Test
  void testEnableUser() throws Exception {
    mvc.perform(post("/users/{userId}:enable", 302).header(TOKEN_HEADER_NAME, TOKEN))
        .andExpect(status().isOk());
  }
}
